...
export const BOOL_SETTINGS = {
  ...
  // off by default: anchor the player's own buff/debuff rows to the movable
  // player frame instead of the classic top-right corner. hud.ts reparents the
  // two aura bars into #player-frame (above it while docked over the action
  // bars, below it once the frame is moved), so they follow the frame's spot
  // and scale. Desktop only; the mobile layout keeps its own aura placement.
