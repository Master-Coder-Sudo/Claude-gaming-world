...
Phase 04 (table router) DONE (2026-06-30: two PURE, server-only modules, NOT wired into the live server until Phase 9). `server/http/path_pattern.ts` (compilePattern = the no-regex routing guard, normalizePath, matchPattern, and the HttpMethod/PatternSegment/CompiledPattern types) + `server/http/router.ts` (createRouter over a Map<HttpMethod,{static:Map,dynamic[]}>, returning the MatchResult union). 55 unit tests; full npm test 611 files / 6411 pass; tsc/biome/build:env/build:server/build all green; ASCII-clean. Reviewers (privacy-security-review, qa-checklist, fresh coverage subagent): 0 BLOCKING; all SHOULD-FIX + NIT applied. See "Phase 4 router contract" below for what Phases 5/7/9/17 inherit. Next: Phase 04 QA (phase-04-qa.md).
