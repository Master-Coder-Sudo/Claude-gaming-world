...
- server/main.ts: the `startServer()` export (Phase 1) and where `createServer`/`listen`/the SIGTERM drain live; the dispatcher selection wired in Phase 9 (the env dispatch flag name and where it is read); and EVERY scattered `process.env.*` read on the boot or request path (list each with its enclosing symbol).
